...
(function KeyboardShortcut() {
  ...
  const binds = {
    ...
    up: {
      callback: () =>
        Spicetify.Player.setVolume(Spicetify.Player.getVolume() + 0.1),
    },
    down: {
      callback: () =>
        Spicetify.Player.setVolume(Spicetify.Player.getVolume() - 0.1),
  ...
  function findActiveIndex(allItems) {
    ...
    const historyURI = Spicetify.Platform.History.location.pathname
      .replace(/^\//, "spotify:")
      .replace(/\//g, ":");
    const activePage = document.querySelector(
      `[aria-describedby="onClickHint${historyURI}"]`,
    );
...
function VimBind() {
  ...
  vimOverlay.style.position =
    baseOverlay.style.position =
    tippyOverlay.style.position =
      "absolute";
  vimOverlay.style.width =
    baseOverlay.style.width =
    tippyOverlay.style.width =
      "100%";
  vimOverlay.style.height =
    baseOverlay.style.height =
    tippyOverlay.style.height =
      "100%";
...
.vim-key {
  ...
  function listenToKeys(event) {
    ...
    if (
      baseOverlay.childNodes.length === 0 &&
      tippyOverlay.childNodes.length === 0
    ) {
      this.deactivate(event);
    }
  }
